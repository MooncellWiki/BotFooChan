...
from src.plugins.bison.types import Category, RawPost, SubUnit, Tag, Target
from src.plugins.bison.utils import ProcessContext, Site
...
async def catch_network_error[**P, R](
    ...
    try:
        ...
        if plugin_config.bison_show_network_warning:
            logger.warning(
                f"network connection error: {type(err)}, url: {err.request.url}"
            )
        return None
...
class Platform(metaclass=PlatformABCMeta, base=True):
    ...
    async def fetch_new_post(
        self, sub_unit: SubUnit
    ) -> list[tuple[SendTarget, list[Post]]]: ...
    ...
    async def dispatch_user_post(
        ...
        res: list[tuple[SendTarget, list[Post]]] = []
...
class NewMessage(MessageProcess, abstract=True):
    ...
    async def filter_common_with_diff(
        ...
        logger.trace(
            f"本次抓取 {len(raw_post_list)} 条，过滤后 {len(filtered_post)} 条，"
            f"新消息 {len(res)} 条"
        )
        return res
    ...
    async def batch_fetch_new_post(
        ...
        if not self.has_target:
            raise RuntimeError(
                "Target without target should not use batch api"
            )  # pragma: no cover
...
class StatusChange(Platform, abstract=True):
    ...
    @abstractmethod
    def compare_status(
        self, target: Target, old_status, new_status
    ) -> list[RawPost]: ...
