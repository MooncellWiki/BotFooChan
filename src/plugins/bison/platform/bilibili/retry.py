...
@dataclass
class RetryAddon[TBilibili: Bilibili]:
    ...
    def record_backoff_finish_time(self):
        self.backoff_finish_time = (
            datetime.now(UTC) + self.backoff_timedelta * self.backoff_count**2
    ...
    def is_in_backoff_time(self):
        ...
        logger.trace(
            f"now: {datetime.now(UTC)}, backoff_finish_time: {self.backoff_finish_time}"
        )
        return datetime.now(UTC) < self.backoff_finish_time
...
async def action_up_to_backoff(
    from_: RetryState, event: RetryEvent, to: RetryState, addon: RetryAddon
) -> ActionReturn:
    ...
    logger.warning(
        f"当前已回避次数: {addon.backoff_count}/{addon.max_backoff_count}, "
        f"本次回避时间至 {addon.backoff_finish_time}"
    )
    ...
    "transitions": {
        ...
        RetryState.BACKOFF: {
            RetryEvent.IN_BACKOFF_TIME: [
                ...
                Transition(
                    action_back_to_refresh,
                    RetryState.REFRESH,
                    {Condition(is_out_backoff_time)},
                ),
            ],
        },
        RetryState.RAISE: {
