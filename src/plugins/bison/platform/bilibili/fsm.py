...
type ActionReturn = Any


@runtime_checkable
class SupportStateOnExit[TAddon](Protocol):
    async def on_exit(self, addon: TAddon) -> None: ...


@runtime_checkable
class SupportStateOnEnter[TAddon](Protocol):
    async def on_enter(self, addon: TAddon) -> None: ...


class Action[TState, TEvent, TAddon](Protocol):
    async def __call__(
        self, from_: TState, event: TEvent, to: TState, addon: TAddon
    ) -> ActionReturn: ...


type ConditionFunc[TAddon] = Callable[[TAddon], Awaitable[bool]]


@dataclass(frozen=True)
class Condition[TAddon]:
...
class Transition[TState, TEvent, TAddon](NamedTuple):
    action: Action[TState, TEvent, TAddon]
    to: TState
    conditions: AbstractSet[Condition[TAddon]] | None = None


class StateGraph[TState, TEvent, TAddon](TypedDict):
    transitions: dict[
        TState,
        dict[
            TEvent,
            Transition[TState, TEvent, TAddon]
            | Sequence[Transition[TState, TEvent, TAddon]],
        ],
    ]
    initial: TState


class FSM[TState, TEvent, TAddon]:
    ...
    async def _core(self) -> AsyncGenerator[ActionReturn, TEvent]:
        ...
        while True:
            ...
            res = await selected_transition.action(
                self.current_state, event, selected_transition.to, self.addon
            )
        ...
        if isinstance(transitions, Transition):
            ...
            for transition in transitions:
                ...
                values = await asyncio.gather(
                    *(condition(self.addon) for condition in transition.conditions)
                )
                ...
                if no_conds:
                    ...
                    raise StateError(
                        f"Invalid event {event} in state {self.current_state}"
                    )
        else:
            raise TypeError(
                f"Invalid transition type: {transitions}, "
                "expected Transition or Sequence[Transition]"
            )
...
def __reset_clear_up[TFSM: FSM, **P](
    func: Callable[Concatenate[TFSM, P], Awaitable[ActionReturn]], auto_start: bool
):
    @wraps(func)
    async def wrapper(
        fsm_self: TFSM, *args: P.args, **kwargs: P.kwargs
    ) -> ActionReturn:
        try:
