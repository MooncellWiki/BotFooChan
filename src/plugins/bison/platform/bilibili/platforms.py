...
from src.plugins.bison.platform.platform import (
    CategoryNotRecognize,
    CategoryNotSupport,
    NewMessage,
    StatusChange,
)
from src.plugins.bison.post.post import Post
from src.plugins.bison.types import ApiError, Category, RawPost, Tag, Target
from src.plugins.bison.utils import decode_unicode_escapes, text_similarity
    ...
    async def get_sub_list(self, target: Target) -> list[DynRawPost]:
        ...
        if res_obj.code == 0:
            if (data := res_obj.data) and (items := data.items):
                ...
                logger.trace(
                    f"用户{target}的动态列表: "
                    f"{':'.join(x.id_str or x.basic.rid_str for x in items)}"
                )
    ...
    def get_tags(self, raw_post: DynRawPost) -> list[Tag]:
        ...
        if desc := raw_post.modules.module_dynamic.desc:
            for node in desc.rich_text_nodes:
                ...
                ) and node_type == "RICH_TEXT_NODE_TYPE_TOPIC":
        ...
        match raw_post.modules.module_dynamic.major:
            ...
            case LiveRecommendMajor(live_rcmd=live_rcmd):
                ...
                return _ParsedMojarPost(
                    ...
                    content=(
                        f"{live_play_info.parent_area_name} {live_play_info.area_name}"
                    ),
                    pics=[live_play_info.cover],
                    url=URL(live_play_info.link)
                    .with_scheme("https")
                    .with_query(None)
                    .human_repr(),
                )
...
class Bilibililive(StatusChange):
    ...
    async def parse(self, raw_post: Info) -> Post:
        ...
        title = (
            f"[{self.categories[raw_post.category].rstrip('提醒')}] {raw_post.title}"
        )
...
Bilibililive.Info.model_rebuild()
