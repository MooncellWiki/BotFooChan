...
from src.plugins.bison.types import Category, RawPost, Target
from src.plugins.bison.utils import Site
...
class ArknightsPost(Post, HTMLContentSupport):
    ...
    async def get_plain_content(self) -> str:
        ...
        content = re.sub(
            r'<span style="color:(#.*?)">(.*?)</span>', r"\2", content
        )  # 去color
        content = re.sub(
            r'<div class="media-wrap image-wrap">(.*?)</div>', "", content
        )  # 去img
...
class Arknights(NewMessage):
    ...
    async def get_sub_list(self, target: Target) -> list[BulletinListItem]:
    ...
    def get_date(self, post: BulletinListItem) -> Any:
        ...
        # 也就是说鹰角可能会在中午就把晚上的公告上传到服务器，
        # 但晚上公告才会显示，而update_at就是中午的时间不会改变
        # 如果指定了get_date，那么get_date会被优先使用,
        # 并在获取到的值超过2小时时忽略这条post，导致其不会被发送
        return None

    def get_category(self, post: BulletinListItem) -> Category:
        return Category(1)
    ...
    async def parse(self, raw_post: BulletinListItem) -> Post:
        ...
        return ArknightsPost(
            ...
            url=(
                url.human_repr()
                if (url := URL(data.jump_link)).scheme.startswith("http")
                else None
            ),
...
class AkVersion(StatusChange):
    ...
    def compare_status(self, target: Target, old_status, new_status):
        res = []
        ...
        if (
            ...
            res.append(
                ArkUpdatePost(title="登录界面维护公告下线（大概是开服了，冲！）")
            )
...
class MonsterSiren(NewMessage):
    ...
    async def parse(self, raw_post: RawPost) -> Post:
        ...
        imgs = [src for x in soup("img") if isinstance(src := x.get("src"), str)]
