...
from src.plugins.bison.theme import theme_manager
from src.plugins.bison.theme.types import ThemeRenderError, ThemeRenderUnsupportError

from .abstract_post import AbstractPost
from .protocol import PlainContentSupport

if TYPE_CHECKING:
    from src.plugins.bison.platform import Platform
        ...
        for theme_name in themes:
            if theme := theme_manager[theme_name]:
                try:
                    ...
                    logger.warning(
                        f"Theme {theme_name} does not support Post of "
                        f"{self.platform.__class__.__name__}: {e}"
                    )
                    continue
            ...
            raise ThemeRenderError(
                f"No theme can render Post of {self.platform.__class__.__name__}"
            )
