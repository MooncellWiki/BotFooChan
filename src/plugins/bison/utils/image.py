...
def to_image(data: str | bytes | Path | BytesIO) -> UniImage:
    """把平台给出的各种图片形态包成 uniseg 的 Image

    saa 的 Image 能直接吃下 url/bytes/Path，uniseg 需要显式区分，统一在这里处理。
    """
    match data:
        case bytes() | BytesIO():
            return UniImage(raw=data)
        case Path():
            return UniImage(path=data)
        case str():
            return UniImage(url=data)
        case _:
            raise TypeError(f"不支持的图片类型: {type(data)}")


async def text_to_image(text: UniText) -> UniImage:
    """使用 htmlrender 将 uniseg 的 Text 渲染为 Image"""
    ...
    render_data = await render_text(text.text)
    return UniImage(raw=render_data.data)
...
async def capture_html(
    ...
    **page_kwargs,
    ...
    goto_timeout: 直接透传给 playwright 的 ``page.goto``，单位毫秒，0 表示不限时。
    它只管打开页面这一步，要给整个渲染过程设上限请在调用侧用 ``asyncio.timeout``。
    """
    ...
    async with get_new_page(
        ...
        await page.goto(url, timeout=goto_timeout, wait_until=wait_until)
