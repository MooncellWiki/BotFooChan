...
from src.plugins.bison.config.db_model import Cookie
...
class CookieClientManager(ClientManager):
    ...
    async def _refresh_anonymous_cookie(self):
        ...
        existing_anonymous_cookies = await config.get_cookie(
    ...
    def _generate_hook(self, cookie: Cookie) -> Callable:
        ...
        async def _response_hook(resp: httpx.Response):
            if resp.status_code == 200:
                ...
                logger.warning(
                    f"请求失败: {cookie.id} {resp.request.url}, "
                    f"状态码: {resp.status_code}"
                )
                cookie.status = "failed"
            cookie.last_usage = db_now()
    ...
    async def _choose_cookie(self, target: Target | None) -> Cookie:
        ...
        available_cookies = (
            cookie for cookie in cookies if cookie.last_usage + cookie.cd < db_now()
        )
