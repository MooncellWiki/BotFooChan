...
from src.plugins.bison.types import Target as T_Target
...
class DBConfig:
    ...
    async def get_cookie(
        ...
        async with get_session() as sess:
            ...
            if target:
                ...
                res = [
                    cookie for cookie in res if cookie.id in ids or cookie.is_universal
                ]
            return res
    ...
    async def update_cookie(self, cookie: Cookie):
        async with get_session() as sess:
            cookie_in_db: Cookie | None = await sess.scalar(
                select(Cookie).where(Cookie.id == cookie.id)
            )
            if not cookie_in_db:
    ...
    async def add_cookie_target(
        self, target: T_Target, platform_name: str, cookie_id: int
    ):
        """通过 cookie_id 可以唯一确定一个 Cookie，
        通过 target 和 platform_name 可以唯一确定一个 Target
        """
        async with get_session() as sess:
    ...
    async def delete_cookie_target(
        self, target: T_Target, platform_name: str, cookie_id: int
    ):
        async with get_session() as sess:
            target_obj = await sess.scalar(
            ...
            cookie_obj = await sess.scalar(select(Cookie).where(Cookie.id == cookie_id))
            await sess.execute(
                delete(CookieTarget).where(
                    CookieTarget.target == target_obj, CookieTarget.cookie == cookie_obj
                )
            )
            await sess.commit()
    ...
    async def delete_cookie_target_by_id(self, cookie_target_id: int):
        async with get_session() as sess:
            await sess.execute(
                delete(CookieTarget).where(CookieTarget.id == cookie_target_id)
            )
            await sess.commit()
