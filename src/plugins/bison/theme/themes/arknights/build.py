...
from src.plugins.bison.theme import Theme, ThemeRenderError, ThemeRenderUnsupportError
from src.plugins.bison.theme.utils import web_embed_image
from src.plugins.bison.utils import text_fletten, to_image

if TYPE_CHECKING:
    from src.plugins.bison.platform.arknights import ArknightsPost
...
class ArknightsTheme(Theme):
    ...
    async def render(self, post: "ArknightsPost"):
        ...
        match banner:
            ...
            case _:
                raise ThemeRenderUnsupportError(
                    "图片类型错误, 期望 str | Path | bytes | BytesIO | None, "
                    f"实际为 {type(banner)}"
                )
        ark_data = ArkData(
        ...
        msgs: list[Segment] = []
        msgs.append(Image(raw=announce_pic))

        if post.url:
            msgs.append(Text(f"前往:{post.url}"))
        if post.images:
            msgs.extend(to_image(img) for img in post.images[1:])
