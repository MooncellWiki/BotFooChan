...
if TYPE_CHECKING:
    from src.plugins.bison.post.abstract_post import AbstractPost
...
class Theme(ABC, BaseModel):
    ...
    async def is_support_render(self, post: "AbstractPost") -> bool:
        ...
        if self.need_browser and not plugin_config.bison_use_browser:
            logger.warning(
                f"Theme {self.name} need browser, but `bison_use_browser` is False"
            )
            return False
        return True
    ...
    async def do_render(self, post: "AbstractPost") -> list[Segment]:
        """真正调用的渲染函数，会对渲染过程进行一些处理"""
        if not await self.is_support_render(post):
            raise ThemeRenderUnsupportError(
                f"Theme [{self.name}] does not support render {post} by support check"
            )
    ...
    @abstractmethod
    async def render(self, post: "AbstractPost") -> list[Segment]:
