...
from src.plugins.bison.utils.site import is_cookie_client_manager
...
async def set_target_user_info(bot: Bot, event: Event, state: T_State):
    # 带上 bot 才能记下 self_id，QQ 官方 bot 的 openid 是按 bot 区分的
    state["target_user_info"] = get_target(event, bot)
...
async def generate_sub_list_text(
    ...
    state: T_State,
    user_info: SendTarget | None = None,
    is_index=False,
    ...
    if is_hide_no_cookie_platfrom:
        sub_list = [
            sub
            for sub in sub_list
            if is_cookie_client_manager(
                platform_manager[sub.target.platform_name].site.client_mgr
            )
        ]
    if not sub_list:
        ...
        if platform := platform_manager.get(sub.target.platform_name):
            ...
            if is_show_cookie:
                target_cookies = await config.get_cookie(
                    target=T_Target(sub.target.target),
