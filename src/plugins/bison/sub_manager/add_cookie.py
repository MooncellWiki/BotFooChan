...
from src.plugins.bison.scheduler import scheduler_dict
from src.plugins.bison.utils.site import CookieClientManager, is_cookie_client_manager
...
def do_add_cookie(add_cookie: type[Matcher]):
    ...
    @add_cookie.handle()
    async def init_promote(state: T_State, event: Event):
            ...
            + "".join(
                [
                    ...
                    if is_cookie_client_manager(
                        platform_manager[platform_name].site.client_mgr
                    )
                ]
            )
            + "要查看全部平台请输入：“全部”\n中止添加cookie过程请输入：“取消”"
    ...
    async def got_cookie(state: T_State, cookie: Message = Arg()):
        client_mgr = cast(
            "CookieClientManager",
            scheduler_dict[platform_manager[state["platform"]].site].client_mgr,
        )
    ...
    async def add_cookie_process(state: T_State):
        ...
        new_cookie = await client_mgr.add_identified_cookie(
            state["cookie"], state["cookie_name"]
        )
