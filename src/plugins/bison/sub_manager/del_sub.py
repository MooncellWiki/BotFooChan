...
def do_del_sub(del_sub: type[Matcher]):
    ...
    @del_sub.handle()
    async def send_list(
        state: T_State, user_info: SendTarget = Arg("target_user_info")
    ):
        ...
        for index, sub in enumerate(sub_list, 1):
            ...
            res += (
                f"{index} {sub.target.platform_name} "
                f"{sub.target.target_name} {sub.target.target}\n"
            )
    ...
    async def do_del(
        ...
        user_info: SendTarget = Arg("target_user_info"),
    ):
        try:
            index = int(index_str)
