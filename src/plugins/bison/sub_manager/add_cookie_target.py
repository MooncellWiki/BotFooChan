...
def do_add_cookie_target(add_cookie_target_matcher: type[Matcher]):
    ...
    async def init_promote_cookie(state: T_State):
        # 获取 site 的所有实名 cookie，再排除掉已经关联的，
        # 剩下的就是可以关联的 cookie
        cookies = await config.get_cookie(
            site_name=state["site"].name, is_anonymous=False
        )
        ...
        if not cookies:
            await add_cookie_target_matcher.finish(
                "当前平台暂无可关联的 Cookie，"
                "请使用“添加cookie”命令添加或检查已关联的 Cookie"
            )
    ...
    @add_cookie_target_matcher.got(
        "cookie_idx", MessageTemplate("{_prompt}"), [handle_cancel]
    )
    ...
    async def add_cookie_target_process(state: T_State):
        ...
        cookie = state["cookie"]
        await add_cookie_target_matcher.finish(
            f"已关联 Cookie: {cookie.cookie_name} 到订阅 "
            f"{state['site'].name} {state['target']['target']}"
        )
