...
def do_del_cookie_target(del_cookie_target: type[Matcher]):
    ...
    async def send_list(state: T_State, event: Event):
        ...
        for index, cookie_target in enumerate(cookie_targets, 1):
            ...
            res += (
                f"{index} {cookie_target.target.platform_name} "
                f"{cookie_target.target.target_name} {friendly_name}\n"
            )
            if res[-1] != "\n":
                res += "\n"
        res += "请输入要删除的关联的序号\n输入'取消'中止"
    ...
    async def do_del(
        ...
        try:
            index = int(index_str)
            await config.delete_cookie_target_by_id(
                state["cookie_target_table"][index]["cookie_target"].id
            )
        except Exception:
