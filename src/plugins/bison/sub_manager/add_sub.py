...
from src.plugins.bison.apis import check_sub_target
from src.plugins.bison.config import config
from src.plugins.bison.config.db_config import SubscribeDupException
from src.plugins.bison.platform import Platform, platform_manager, unavailable_paltforms
...
def do_add_sub(add_sub: type[Matcher]):
    ...
    async def got_id(bot: Bot, event: Event, state: T_State, raw_id: Message = Arg()):
    ...
    async def add_sub_process(
        state: T_State, user: SendTarget = Arg("target_user_info")
    ):
        try:
