...
from src.plugins.bison.send import send_msgs
from src.plugins.bison.types import SubUnit, Target
from src.plugins.bison.utils import ClientManager, ProcessContext, Site
from src.plugins.bison.utils.site import SkipRequestException
...
class Scheduler:
    ...
    def __init__(
        ...
        logger.info(
            ...
            f"{self.scheduler_config.schedule_type} "
            f"{self.scheduler_config.schedule_setting}"
        )
        scheduler.add_job(
            self.exec_fetch,
    ...
    async def exec_fetch(self):
        if not (schedulable := await self.get_next_schedulable()):
            return
        logger.trace(
            f"scheduler {self.name} fetching next target: "
            f"[{schedulable.platform_name}]{schedulable.target}"
        )

        context = ProcessContext(self.client_mgr)
        try:
            await self._run_schedulable_fetch(context, schedulable)
        finally:
            await context.cleanup()

    async def _run_schedulable_fetch(
        self, context: ProcessContext, schedulable: Schedulable
    ) -> None:
        ...
        try:
            if schedulable.use_batch:
                ...
                to_send = await platform_obj.do_fetch_new_post(
                    SubUnit(schedulable.target, send_userinfo_list)
                )
    ...
    def insert_new_schedulable(self, platform_name: str, target: Target):
        ...
        logger.info(
            f"insert [{platform_name}]{target} to "
            f"Schduler({self.scheduler_config.name})"
        )
